...
///
/// ``AsyncFuncPath`` is available as an alias.
///
@dynamicMemberLookup
public struct AsyncFunctionPath<Root, Input, Return> {
    ...
    /// Appends a transformation to the current `AsyncFunctionPath` using another `FunctionPathWithInput`.
    ///
    ...
    /// types as the original but with an `AppendedReturn` type that reflects the return type of the provided `FunctionPathWithInput`.
    ///
    ...
    /// Appends a transformation to the current `AsyncFunctionPath` using another `ThrowingFunctionPathWithInput`.
    ///
    /// This method allows for the composition of two function paths, effectively chaining their behavior.
    ...
    /// Appends a transformation to the current `AsyncFunctionPath` using another `AsyncFunctionPathWithInput`.
    ///
    /// This method allows for the composition of two function paths, effectively chaining their behavior.
    ...
    /// Appends a transformation to the current `AsyncFunctionPath` using another `AsyncThrowingFunctionPathWithInput`.
    ///
    /// This method allows for the composition of two function paths, effectively chaining their behavior.
    ...
    /// Appends a transformation to the current `AsyncFunctionPath` using a `KeyPath`.
    ///
    ...
    /// Appends a transformation to the current `AsyncFunctionPath` using a `KeyPath`.
    ///
    /// This version of the `appending` method creates a new `AsyncFunctionPath` instance, extending the original function's behavior by applying the specified `KeyPath`.
    ///
    /// - Parameter keyPath: A `KeyPath` from the `Return` type of the current function path to the desired `AppendedReturn` type.
    ///   This keyPath specifies the property to be extracted or transformed from the `Return` type.
    ///
    /// - Returns: A new `AsyncFunctionPath<Root, Input, AppendedReturn>` instance.
    ...
    ///
    /// - Returns: A closure of type `(Root) async -> Return`.
    ...
    /// Captures the input arguments of a function and turns this path into a instance of `AsyncFunctionPathWithInput`
    ///
    /// - Parameter input: Input value of the function indicated by this path.
    ///     With `@autoclosure` attribute, this input is evaluated at function execution time
    ///
    /// - Returns: A instance of `AsyncFunctionPathWithInput`.
    ///
    public func callAsFunction(
        _ input: @escaping @autoclosure () -> Input
